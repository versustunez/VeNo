...
void Logging::debug(
  ...
  vprintf(
      Logging::format(newLine, PrintType::DBG, log, file, func).c_str(), args);
  va_end(args);
}
...
void Logging::log(
  ...
  auto log = va_arg(args, const char *);
  vprintf(
      Logging::format(newLine, PrintType::LOG, log, file, func).c_str(), args);
  va_end(args);
}
...
void Logging::warn(
  ...
  auto log = va_arg(args, const char *);
  vprintf(
      Logging::format(newLine, PrintType::WARN, log, file, func).c_str(), args);
  va_end(args);
}

std::string Logging::format(bool newLine, PrintType type, const char *log,
...
std::string Logging::getPrefix(PrintType type, const char *s) {
  ...
  case PrintType::ERROR:
    return "\033[1;" + std::to_string(ERROR_COLOR) + "m[ERROR][" +
           std::string(s) + "] >> ";
  case PrintType::DBG:
    return "\033[1;" + std::to_string(DEBUG_COLOR) + "m[DEBUG][" +
           std::string(s) + "] >> ";
  case PrintType::WARN:
    return "\033[1;" + std::to_string(WARN_COLOR) + "m[WARN][" +
           std::string(s) + "] >> ";
  case PrintType::LOG:
  default:
