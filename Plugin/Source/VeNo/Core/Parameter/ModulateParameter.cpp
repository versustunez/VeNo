...
namespace VeNo::Core
{
ModulateParameter::ModulateParameter (std::string name, std::string showName, double min, double max, double value, size_t id)
...
void ModulateParameter::addValueVoice (int voice, double value)
{
    ...
    m_values[voice] = VUtils::Math::clamp (m_matrix[voice] * (m_preMinMax) + m_min, m_min, m_max);
}
void ModulateParameter::addValue (double value)
{
    m_matrixPos += value;
    m_value = VUtils::Math::clamp (m_matrixPos * (m_preMinMax) + m_min, m_min, m_max);
    for (int i = 0; i < MAX_VOICES; ++i)
    {
        m_matrix[i] += value;
        m_values[i] = VUtils::Math::clamp (m_matrix[i] * (m_preMinMax) + m_min, m_min, m_max);
    }
}
