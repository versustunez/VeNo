...
void LookHandler::drawTextEditorOutline(
...
void LookHandler::drawToggleButton(juce::Graphics &g,
    juce::ToggleButton &button, bool shouldDrawButtonAsHighlighted,
    bool shouldDrawButtonAsDown) {
  getLook()->drawToggleButton(
...
void LookHandler::drawTabAreaBehindFrontButton(
    juce::TabbedButtonBar &, juce::Graphics &g, int w, int h) {
  g.setColour(theme->getDirectColor(Colors::bgTwo).withAlpha(0.5f));
  g.drawLine(0, h, w, h);
}

juce::PopupMenu::Options LookHandler::getOptionsForComboBoxPopupMenu(
    juce::ComboBox &box, juce::Label &) {
...
void LookHandler::drawPopupMenuItem(juce::Graphics &g,
...
void LookHandler::drawPopupMenuBackground(
