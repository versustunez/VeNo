...
void FlatLook::drawRotarySlider(juce::Graphics &g, int x, int y, int width,
    int height, float sliderPosProportional, float rotaryStartAngle,
  ...
  outerArc.addCentredArc(
      0, 0, radius, radius, 0, -MAX_RADIAN, MAX_RADIAN, true);
  ...
  juce::Path arc;
  arc.addCentredArc(0, 0, radius, radius, 0, -MAX_RADIAN,
...
void FlatLook::drawTextEditorOutline(
    juce::Graphics &graphics, int width, int height, juce::TextEditor &) {
  graphics.setGradientFill(
      juce::ColourGradient::horizontal(theme->getDirectColor(Colors::accentTwo),
          0, theme->getDirectColor(Colors::accent), (float)width));
  auto p = 10.0f;
  graphics.drawLine(p, (float)height, (float)width - p, (float)height, 0.9f);
}

void FlatLook::drawToggleButton(juce::Graphics &graphics,
    juce::ToggleButton &button, bool, bool shouldDrawButtonAsDown) {
  int height = button.getHeight();
  int width = button.getWidth();
  auto text = button.getButtonText();
  auto *parent = dynamic_cast<VeNo::GUI::Switch *>(button.getParentComponent());
  ...
  graphics.drawText(
      text, l, l, width - p, height - p, juce::Justification::centred, true);
}

void FlatLook::drawTabButton(
    juce::TabBarButton &button, juce::Graphics &graphics, bool, bool) {
...
void FlatLook::drawLabel(juce::Graphics &graphics, juce::Label &label) {
  if (!label.isBeingEdited()) {
    ...
    graphics.drawFittedText(label.getText(), textArea,
